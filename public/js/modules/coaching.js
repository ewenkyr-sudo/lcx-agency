...
async function renderCoaching() {
  ...
  container.innerHTML = `
    <div class="coaching-kpi-bar">
      <div class="coaching-kpi"><div class="coaching-kpi-value" style="color:var(--accent-blue-light)">${students.length}</div><div class="coaching-kpi-label">${t('coaching.students_active')}</div></div>
      ...
      <div class="coaching-kpi"><div class="coaching-kpi-value" style="color:${pendingCalls.length > 0 ? 'var(--red)' : 'var(--text-tertiary)'}">${pendingCalls.length}</div><div class="coaching-kpi-label">${t('coaching.calls_pending')}</div></div>
...
function renderCoachingStudentList() {
  ...
  list.innerHTML = filtered.map(function(s) {
    ...
    return '<div class="coaching-student-card' + (_selectedStudentId === s.user_id ? ' selected' : '') + '" onclick="selectCoachingStudent(' + s.user_id + ')">'
      ...
      + '<div class="coaching-student-meta"><span class="coaching-badge coaching-badge-' + prog + '">' + prog.toUpperCase() + '</span> <span style="color:' + (stepObj.key === 'traffic' ? 'var(--green)' : 'var(--accent-blue-light)') + '">' + stepObj.icon + ' ' + stepObj.label + '</span></div>'
      + '</div>'
      + (rev > 0 ? '<span style="font-size:11px;font-weight:700;color:var(--green)">$' + rev.toFixed(0) + '</span>' : '')
      + '</div>';
  }).join('') || '<div style="color:var(--text-tertiary);text-align:center;padding:24px">' + t('coaching.no_students') + '</div>';
}
...
function renderCoachingTabContent() {
  ...
  if (_coachingTab === 'overview') {
    ...
    body.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:16px;margin-bottom:20px">'
      ...
      + '<div style="background:var(--bg-base);padding:12px;border-radius:10px;text-align:center"><div style="font-size:20px;font-weight:800;color:' + (pendingCall ? 'var(--red)' : 'var(--text-tertiary)') + '">' + calls.length + '</div><div style="font-size:10px;color:var(--text-tertiary)">' + t('coaching.call_requests') + '</div></div>'
      + '</div>'
      // Call requests
      + (pendingCall ? '<div style="background:var(--red-bg);border:1px solid rgba(251,113,133,0.2);border-radius:10px;padding:12px;margin-bottom:16px"><strong style="color:var(--red)">' + t('coaching.call_pending') + '</strong><div style="font-size:13px;color:var(--text-secondary);margin-top:4px">' + (pendingCall.message || '-') + '</div><div style="display:flex;gap:6px;margin-top:8px"><button class="btn btn-primary" style="font-size:11px;padding:4px 12px" onclick="handleCallRequest(' + pendingCall.id + ',\'accepted\')">' + t('planning.accept') + '</button><button class="btn" style="font-size:11px;padding:4px 12px;background:var(--red-bg);color:var(--red);border:none;cursor:pointer" onclick="handleCallRequest(' + pendingCall.id + ',\'refused\')">' + t('planning.refuse') + '</button></div></div>' : '')
      // Outreach assignments
      + '<div style="background:var(--bg-base);padding:14px;border-radius:10px;margin-bottom:12px">'
      + '<div style="font-size:12px;color:var(--text-tertiary);margin-bottom:8px">' + t('coaching.outreach_assigned') + '</div>'
      ...
      + (function() {
        ...
        var students = _coachingData.students || [];
        return '<div style="background:var(--bg-base);padding:14px;border-radius:10px;margin-bottom:12px">'
          + '<div style="font-size:12px;color:var(--text-tertiary);margin-bottom:8px">' + t('coaching.outreach_shared') + '</div>'
          + '<div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">'
          ...
          + '<select onchange="addOutreachPair(' + s.user_id + ',this.value);this.value=\'\'" style="font-size:11px;padding:3px 8px;background:var(--bg);border:1px solid var(--border);color:var(--text);border-radius:6px;cursor:pointer"><option value="">+ ' + t('coaching.pair_btn') + '</option>'
    ...
    body.innerHTML = '<div id="coaching-outreach-content" style="color:var(--text-tertiary);text-align:center;padding:16px">' + t('common.loading') + '</div>';
    loadCoachingStudentOutreach(_selectedStudentId);
  } else if (_coachingTab === 'planning') {
    body.innerHTML = '<div id="coaching-planning-content" style="color:var(--text-tertiary);text-align:center;padding:16px">' + t('common.loading') + '</div>';
    loadCoachingStudentPlanning(_selectedStudentId);
  } else if (_coachingTab === 'tasks') {
    body.innerHTML = '<div id="coaching-tasks-content" style="color:var(--text-tertiary);text-align:center;padding:16px">' + t('common.loading') + '</div>';
    ...
    body.innerHTML = '<h4 style="font-size:14px;font-weight:700;margin-bottom:12px;color:var(--accent-blue-light)">' + t('coaching.student_revenue') + '</h4>'
      ...
      + (revs.length > 0 ? revs.map(function(r) {
        ...
        }).join('') : '<tr><td colspan="4" style="text-align:center;color:var(--text-tertiary);padding:20px">' + t('coaching.no_revenue') + '</td></tr>')
      + '</tbody></table>'
      + '<h4 style="font-size:14px;font-weight:700;margin:20px 0 12px;color:var(--accent-blue-light)">' + t('coaching.recruit_models') + '</h4>'
    ...
    body.innerHTML = '<div style="display:flex;gap:0;height:350px;border-radius:10px;overflow:hidden;border:1px solid var(--border)">'
      ...
      + '<div style="display:flex;gap:6px"><input type="text" id="admin-chat-input" class="form-input" placeholder="' + t('coaching.reply_placeholder') + '" style="flex:1" onkeydown="if(event.key===\'Enter\')sendAdminMessage()"><button class="btn btn-primary" onclick="sendAdminMessage()" style="padding:6px 12px;font-size:12px">' + t('common.send') + '</button></div></div></div></div>';
...
async function loadCoachingStudentOutreach(studentUserId, btn) {
  ...
  document.querySelectorAll('.coaching-student-btn').forEach(b => { b.style.background = 'var(--bg-elevated)'; b.style.color = 'var(--text-secondary)'; });
...
async function renderCoachingOutreach() {
  ...
  container.innerHTML = ''
    ...
    + (usEnabled ? '<div style="display:flex;gap:8px;margin-bottom:16px">'
      + '<button onclick="coachingOutreachMarket=\'fr\';renderCoachingOutreach()" style="padding:8px 20px;border-radius:8px;border:none;cursor:pointer;font-weight:700;font-size:13px;background:' + (coachingOutreachMarket==='fr'?'var(--accent)':'var(--bg-elevated)') + ';color:' + (coachingOutreachMarket==='fr'?'white':'var(--text-secondary)') + '">FR</button>'
      + '<button onclick="coachingOutreachMarket=\'us\';renderCoachingOutreach()" style="padding:8px 20px;border-radius:8px;border:none;cursor:pointer;font-weight:700;font-size:13px;background:' + (coachingOutreachMarket==='us'?'var(--accent)':'var(--bg-elevated)') + ';color:' + (coachingOutreachMarket==='us'?'white':'var(--text-secondary)') + '">US</button>'
      + '</div>' : '')
    // Stats
    + '<div class="stats-grid" style="margin-bottom:16px">'
    + '<div class="stat-card"><div class="stat-value">' + (stats.leads_today || 0) + '</div><div class="stat-label">' + t('coaching.leads_today') + '</div></div>'
    + '<div class="stat-card"><div class="stat-value" style="color:var(--blue)">' + (stats.dm_sent_today || 0) + '</div><div class="stat-label">' + t('coaching.dms_today') + '</div></div>'
    + '<div class="stat-card"><div class="stat-value">' + (stats.dm_sent || 0) + '</div><div class="stat-label">' + t('coaching.dms_total') + '</div></div>'
    + '<div class="stat-card"><div class="stat-value" style="color:var(--yellow)">' + (stats.talking_warm || 0) + '</div><div class="stat-label">' + t('coaching.talking_warm') + '</div></div>'
    + '<div class="stat-card"><div class="stat-value" style="color:var(--green)">' + (stats.call_booked || 0) + '</div><div class="stat-label">' + t('coaching.call_booked_label') + '</div></div>'
    + '<div class="stat-card"><div class="stat-value" style="color:var(--accent-blue-light)">' + (stats.reply_rate || 0) + '%</div><div class="stat-label">' + t('coaching.reply_rate') + '</div></div>'
    + '</div>'
    // Actions
    + '<div style="display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap">'
    + '<button class="btn btn-primary" onclick="showCoachingLeadForm()" style="font-size:12px">' + t('common.new_lead') + '</button>'
    + '</div>'
    ...
    + ['all','to-send','sent','talking-cold','talking-warm','call-booked','signed'].map(function(fi) {
      return '<button class="btn coaching-lead-filter" onclick="coachingOutreachFilter=\'' + fi + '\';renderCoachingLeadTable();document.querySelectorAll(\'.coaching-lead-filter\').forEach(b=>{b.style.background=\'var(--bg-elevated)\';b.style.color=\'var(--text-secondary)\'});this.style.background=\'var(--accent)\';this.style.color=\'white\'" style="font-size:12px;padding:6px 14px;border-radius:20px;background:' + (coachingOutreachFilter===fi?'var(--accent)':'var(--bg-elevated)') + ';color:' + (coachingOutreachFilter===fi?'white':'var(--text-secondary)') + ';border:none;cursor:pointer">' + (fi==='all'?t('common.all'):(leadStatusColors[fi]?.label||fi)) + '</button>';
    }).join('')
    + '</div>'
    // Recherche
    + '<div style="margin-bottom:12px"><input type="text" id="coaching-lead-search" class="form-input" placeholder="' + t('coaching.search_username') + '" oninput="debouncedRenderCoachingLeadTable()" style="max-width:350px"></div>'
    // Tableau
...
function coachingInlineSelect(leadId, field, currentValue, optType) {
  ...
  var style = 'background:var(--bg-base);color:var(--text);border:1px solid var(--border);padding:4px 6px;border-radius:6px;font-size:11px;cursor:pointer;font-family:inherit;min-height:28px;width:100%';
...
function renderCoachingLeadTable() {
  ...
  tbody.innerHTML = filtered.map(function(l, idx) {
    ...
    return '<tr><td data-label="#" style="color:var(--text-tertiary);font-size:12px">' + (filtered.length - idx) + '</td>'
      ...
      + '<td data-label="Ajouté par" class="mc-half" style="font-size:11px;color:var(--accent-blue-light)">' + (l.modified_by_name || l.added_by_name || '-') + '</td>'
      + '<td data-label="Notes" class="mc-full"><input type="text" value="' + (l.notes||'').replace(/"/g,'&quot;') + '" onchange="updateCoachingLead(' + l.id + ',{notes:this.value})" style="background:var(--bg);border:1px solid var(--border);color:var(--text-secondary);padding:4px 8px;border-radius:6px;font-size:12px;width:100%;font-family:inherit"></td>'
...
async function deleteCoachingLead(id) {
  if (!(await confirmDelete(t('coaching.delete_lead_confirm')))) return;
...
async function deleteAllCoachingLeads() {
  ...
  if (!(await confirmDelete(t('coaching.delete_all_leads_confirm'))) ) return;
  ...
  if (res.ok) { showToast(t('coaching.leads_deleted'), 'success'); renderCoachingOutreach(); }
}
...
function showCoachingLeadForm() {
  ...
  wrap.innerHTML = '<div class="panel" style="padding:16px;margin-bottom:16px;background:var(--bg-base);border-radius:10px">'
    ...
    + '<div><label style="font-size:11px;color:var(--text-tertiary)">Script</label><select id="cl-script" class="form-input"><option value="">-- Script --</option>' + scriptOpts + '</select></div>'
    + '<div><label style="font-size:11px;color:var(--text-tertiary)">Compte</label><select id="cl-account" class="form-input"><option value="">-- Compte --</option>' + accountOpts + '</select></div>'
    + '<div><label style="font-size:11px;color:var(--text-tertiary)">Notes</label><input type="text" id="cl-notes" class="form-input" placeholder="Notes..."></div>'
    + '</div>'
    + '<div style="margin-top:10px;display:flex;gap:8px"><button class="btn btn-primary" style="font-size:12px" onclick="addCoachingLead()">' + t('common.add') + '</button><button class="btn" style="font-size:12px;background:var(--bg);color:var(--text-secondary);border:none;cursor:pointer" onclick="document.getElementById(\'coaching-lead-form-wrap\').innerHTML=\'\'">' + t('common.cancel') + '</button></div>'
    + '</div>';
}

async function addCoachingLead() {
  var username = document.getElementById('cl-username').value.trim();
  if (!username) return showToast(t('coaching.username_required'), 'error');
...
async function loadCoachingStudentPlanning(studentUserId, btn) {
  ...
  document.querySelectorAll('.coaching-planning-btn').forEach(b => { b.style.background = 'var(--bg-elevated)'; b.style.color = 'var(--text-secondary)'; });
  if (btn) { btn.style.background = 'var(--accent)'; btn.style.color = 'white'; }
...
async function renderCoachingPlanning() {
  ...
  let html = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">'
    + '<button onclick="coachingPlanningNavigate(-1)" style="padding:6px 12px;background:var(--bg-base);color:var(--text);border:none;cursor:pointer;border-radius:8px;font-size:14px;font-family:inherit">‹</button>'
  ...
  days.forEach(d => {
    ...
    html += '<div style="background:' + (isToday ? 'rgba(59,130,246,0.08)' : 'var(--bg-elevated)') + ';border-radius:10px;padding:12px 16px;border:1px solid ' + (isToday ? 'var(--accent)' : 'var(--border)') + '">'
      ...
      + '<button onclick="coachingQuickAddShift(\'' + dk + '\')" style="background:none;border:none;color:var(--accent);cursor:pointer;font-size:11px;font-weight:600">' + t('planning.add_shift') + '</button>'
      + '<button onclick="coachingQuickAddTask(\'' + dk + '\')" style="background:none;border:none;color:var(--green);cursor:pointer;font-size:11px;font-weight:600">' + t('planning.add_task') + '</button>'
      + '</div></div>';

    if (isLeave) {
      html += '<div style="padding:8px 12px;background:var(--red-bg);border-radius:6px;color:var(--red);font-weight:600;font-size:12px">' + t('coaching.congé_label') + '</div>';
    } else {
      ...
      if (dayShifts.length === 0) {
        html += '<div style="color:var(--text-tertiary);font-size:11px;font-style:italic">' + t('coaching.no_shift_task') + '</div>';
      }
    }
    html += '</div>';
  });
  ...
  html += '</div>'
    + '<div style="margin-top:12px;padding:12px 16px;background:var(--bg-base);border-radius:8px;display:flex;justify-content:space-between;align-items:center">'
    + '<span style="font-size:12px;color:var(--text-secondary)">' + t('coaching.week_total') + '</span>'
...
async function coachingQuickAddShift(dateStr) {
  const type = prompt(t('coaching.shift_prompt'), 'morning');
  ...
  showToast(t('coaching.shift_added'), 'success');
  renderCoachingPlanning();
}
...
async function coachingQuickAddTask(dateStr) {
  ...
  showToast(t('coaching.task_added_planning'), 'success');
  renderCoachingPlanning();
}
...
async function loadCoachingStudentTasks(studentUserId, btn) {
  ...
  document.querySelectorAll('.coaching-tasks-btn').forEach(b => { b.style.background = 'var(--bg-elevated)'; b.style.color = 'var(--text-secondary)'; });
  if (btn) { btn.style.background = 'var(--accent)'; btn.style.color = 'white'; }
...
async function renderCoachingTasks() {
  ...
  const statusColors = { pending: { bg: 'var(--blue-bg)', color: 'var(--blue)', label: t('planning.pending') }, in_progress: { bg: 'var(--yellow-bg)', color: 'var(--yellow)', label: t('tasks.in_progress_label') }, completed: { bg: 'var(--green-bg)', color: 'var(--green)', label: t('coaching.status_completed') } };
  ...
  function card(t) {
    ...
    return '<div style="background:var(--bg-base);padding:12px;border-radius:8px;border-left:4px solid ' + borderColor + ';position:relative">'
      ...
      + '</div>'
      + (t.description ? '<div style="font-size:11px;color:var(--text-secondary);margin-bottom:6px">' + t.description + '</div>' : '')
      ...
      + '<option value="pending"' + (t.status==='pending'?' selected':'') + ' style="background:var(--bg2);color:var(--text)">' + window.t('coaching.status_pending') + '</option>'
      + '<option value="in_progress"' + (t.status==='in_progress'?' selected':'') + ' style="background:var(--bg2);color:var(--text)">' + window.t('coaching.status_in_progress') + '</option>'
      + '<option value="completed"' + (t.status==='completed'?' selected':'') + ' style="background:var(--bg2);color:var(--text)">' + window.t('coaching.status_completed') + '</option></select>'
      + (dl ? '<div style="color:' + (overdue ? 'var(--red);font-weight:600' : 'var(--text-tertiary)') + '">📅 ' + dl + (overdue ? ' ' + window.t('coaching.overdue') : '') + '</div>' : '')
      + '</div></div>';
  }

  container.innerHTML = '<div style="display:flex;justify-content:flex-end;margin-bottom:12px"><button class="btn btn-primary" style="font-size:12px;padding:6px 12px" onclick="coachingShowTaskForm()">' + t('coaching.new_task_btn') + '</button></div>'
    + '<div id="coaching-task-form-wrap"></div>'
    + '<div style="margin-bottom:12px"><h4 style="font-size:12px;font-weight:700;color:var(--accent-blue-light);margin-bottom:8px">' + t('coaching.todo_section') + ' (' + pending.length + ')</h4>'
    + (pending.length === 0 ? emptyStateHTML('clipboard', t('student.no_task')) : '<div style="display:grid;gap:8px">' + pending.map(card).join('') + '</div>')
    + '</div>'
    + (completed.length > 0 ? '<div><h4 style="font-size:12px;font-weight:700;color:var(--text-secondary);margin-bottom:8px">' + t('coaching.done_section') + ' (' + completed.length + ')</h4><div style="display:grid;gap:8px">' + completed.map(card).join('') + '</div></div>' : '');
}
...
function coachingShowTaskForm() {
  ...
  wrap.innerHTML = '<div class="panel" style="padding:14px;margin-bottom:12px;background:var(--bg2)">'
    ...
    + '<div><label style="font-size:11px;color:var(--text-tertiary);display:block;margin-bottom:4px">' + t('student.deadline_label') + '</label><input type="date" id="ct-deadline" class="form-input"></div>'
    + '</div>'
    + '<div style="margin-top:10px;display:flex;gap:8px"><button class="btn btn-primary" style="font-size:12px" onclick="coachingAddTask()">' + t('common.add') + '</button><button class="btn" style="font-size:12px;background:var(--bg-base);color:var(--text-secondary);border:none;cursor:pointer" onclick="document.getElementById(\'coaching-task-form-wrap\').innerHTML=\'\'">' + t('common.cancel') + '</button></div>'
    + '</div>';
}

async function coachingAddTask() {
  const title = document.getElementById('ct-title').value.trim();
  if (!title) { showToast(t('toast.title_required'), 'error'); return; }
  ...
  if (res.ok) {
    showToast(t('coaching.task_added'), 'success');
...
async function coachingDeleteTask(id) {
  if (!(await confirmDelete(t('coaching.delete_task_confirm')))) return;
...
function showAddObjectiveForm() {
  ...
  wrap.innerHTML = '<div style="background:var(--bg-base);padding:14px;border-radius:10px;margin-bottom:12px;display:flex;gap:10px;flex-wrap:wrap;align-items:end">'
    + '<div><label style="font-size:11px;color:var(--text-tertiary)">' + t('coaching.student_label') + '</label><select id="obj-student" class="form-input" style="font-size:12px">' + studentOpts + '</select></div>'
    + '<div><label style="font-size:11px;color:var(--text-tertiary)">' + t('coaching.obj_type') + '</label><select id="obj-type" class="form-input" style="font-size:12px"><option value="leads">' + t('coaching.obj_leads') + '</option><option value="dms">' + t('coaching.obj_dms') + '</option><option value="calls">' + t('coaching.obj_calls') + '</option><option value="custom">' + t('coaching.obj_other') + '</option></select></div>'
    + '<div><label style="font-size:11px;color:var(--text-tertiary)">' + t('common.description') + '</label><input type="text" id="obj-desc" class="form-input" style="font-size:12px" placeholder="' + t('coaching.obj_desc_placeholder') + '"></div>'
    + '<div><label style="font-size:11px;color:var(--text-tertiary)">' + t('coaching.obj_target') + '</label><input type="number" id="obj-target" class="form-input" style="font-size:12px;width:80px" placeholder="100"></div>'
    + '<button class="btn btn-primary" style="font-size:12px;padding:8px 14px" onclick="addAdminObjective()">' + t('common.add') + '</button>'
    + '</div>';
}
...
async function openAdminChat(userId) {
  ...
  if (res.ok) {
    ...
    container.innerHTML = msgs.map(m => {
      ...
      return '<div style="display:flex;justify-content:' + (isMe ? 'flex-end' : 'flex-start') + '"><div style="max-width:70%;padding:8px 12px;border-radius:' + (isMe ? '12px 12px 4px 12px' : '12px 12px 12px 4px') + ';background:' + (isMe ? 'var(--accent)' : 'var(--bg-elevated)') + ';color:' + (isMe ? 'white' : 'var(--text)') + ';font-size:13px">' + m.content + '<div style="font-size:10px;margin-top:3px;opacity:0.6">' + new Date(m.created_at).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'}) + '</div></div></div>';
    }).join('');
