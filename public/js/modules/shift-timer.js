...
function renderShiftWidget() {
  ...
  if (!_activeShift) {
    ...
    container.innerHTML = '<div style="text-align:center;padding:40px">'
      ...
      + '<div style="color:var(--text3);font-size:13px;margin-top:12px">' + t('shift.start_desc') + '</div>'
      + '</div>';
  ...
  container.innerHTML = ''
    ...
    + '<div style="background:linear-gradient(135deg,rgba(168,85,247,0.15),rgba(34,211,238,0.1));border:1px solid var(--accent);border-radius:14px;padding:20px;margin-bottom:20px">'
    + '<div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px">'
    + '<div><div style="font-size:12px;color:var(--text3);text-transform:uppercase;margin-bottom:4px">' + t('shift.active_shift') + '</div>'
    + '<div style="font-size:13px;color:var(--accent2)">' + models + '</div></div>'
    + '<div style="text-align:center"><div style="font-size:12px;color:var(--text3);margin-bottom:4px">⏱️ ' + t('shift.time_remaining') + '</div>'
    + '<div id="shift-timer-display" style="font-size:32px;font-weight:800;font-variant-numeric:tabular-nums;letter-spacing:1px"></div></div>'
    + '<button class="btn" style="background:var(--red-bg);color:var(--red);border:none;padding:10px 20px;cursor:pointer;border-radius:10px;font-weight:700" onclick="showEndShiftModal()">'
    + '🔴 ' + t('shift.end_shift') + '</button>'
    + '</div></div>'

    // Revenue inputs
    + '<div class="panel" style="padding:20px;margin-bottom:20px">'
    + '<h3 style="font-size:15px;font-weight:700;color:var(--accent2);margin-bottom:16px">' + t('shift.shift_revenue') + '</h3>'
    + '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:12px">'
    + '<div><label style="font-size:11px;color:var(--text3);display:block;margin-bottom:4px">PPV ' + t('shift.sent') + '</label>'
    ...
    + '<div><label style="font-size:11px;color:var(--text3);display:block;margin-bottom:4px">PPV ' + t('shift.sold') + '</label>'
    ...
    + '<div><label style="font-size:11px;color:var(--text3);display:block;margin-bottom:4px">' + t('shift.ppv_revenue') + ' ($)</label>'
    ...
    + '<div><label style="font-size:11px;color:var(--text3);display:block;margin-bottom:4px">' + t('shift.tips_revenue') + ' ($)</label>'
    ...
    + '</div>'
    + '<div style="margin-top:12px"><label style="font-size:11px;color:var(--text3);display:block;margin-bottom:4px">' + t('common.notes') + '</label>'
    ...
    + '<div style="font-size:11px;color:var(--text3);margin-top:8px">💾 ' + t('shift.auto_save') + '</div>'
    + '</div>';
...
function showStartShiftModal() {
  var modelCheckboxes = (allModels || []).map(function(m) {
    return '<label style="display:flex;align-items:center;gap:8px;padding:8px 12px;background:var(--bg3);border-radius:8px;cursor:pointer;font-size:13px"><input type="checkbox" class="shift-model-cb" value="' + m.id + '" style="width:18px;height:18px;cursor:pointer"> ' + m.name + '</label>';
  }).join('');
...
function showEndShiftModal() {
  ...
  var html = '<div class="modal-overlay show" id="shift-end-modal" onclick="if(event.target===this)this.remove()">'
    ...
    + '<div style="font-size:13px;color:var(--text3);margin-bottom:8px">' + t('shift.recap') + '</div>'
    + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">'
    + '<div><div style="font-size:20px;font-weight:800">' + elapsedH + 'h ' + elapsedM + 'min</div><div style="font-size:11px;color:var(--text3)">' + t('shift.duration') + '</div></div>'
    + '<div><div style="font-size:20px;font-weight:800;color:var(--green)">$' + totalRev.toFixed(2) + '</div><div style="font-size:11px;color:var(--text3)">' + t('shift.total_revenue') + '</div></div>'
    + '<div><div style="font-size:16px;font-weight:700">' + ppvCount + ' → ' + ppvSold + '</div><div style="font-size:11px;color:var(--text3)">PPV ' + t('shift.sent') + '/' + t('shift.sold') + '</div></div>'
    + '<div><div style="font-size:16px;font-weight:700">$' + ppvTotal.toFixed(2) + ' / $' + tipsTotal.toFixed(2) + '</div><div style="font-size:11px;color:var(--text3)">PPV / Tips</div></div>'
    + '</div></div>'
...
async function renderActiveShiftsWidget() {
  ...
  try {
    ...
    if (shifts.length === 0) {
      el.innerHTML = '<div style="color:var(--text3);font-size:13px;text-align:center;padding:16px">' + t('shift.no_active_shifts') + '</div>';
      return;
    }
    ...
    el.innerHTML = '<div style="display:grid;gap:8px">' + shifts.map(function(s) {
      ...
      return '<div style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:var(--bg3);border-radius:10px;border-left:3px solid ' + dotColor + '">'
        ...
        + '<div style="flex:1;min-width:0"><strong style="font-size:13px">' + s.chatter_name + '</strong><div style="font-size:11px;color:var(--text3)">' + s.model_name + '</div></div>'
