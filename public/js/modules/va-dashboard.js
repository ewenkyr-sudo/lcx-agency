...
async function renderVA() {
  ...
  try {
    ...
    else container.innerHTML = '<div style="color:var(--text-tertiary);text-align:center;padding:40px">' + t('va.no_va') + '</div>';
  } catch(e) {
...
async function renderVADashboard(container) {
  ...
  if (clockedIn) {
    ...
    clockHTML = '<div style="display:flex;align-items:center;gap:12px;padding:14px 20px;background:linear-gradient(135deg,rgba(34,197,94,0.08),rgba(6,182,212,0.04));border:1px solid rgba(34,197,94,0.2);border-radius:var(--radius-lg)">'
      + '<div class="pulse-dot" style="width:10px;height:10px"></div>'
      + '<span style="font-size:14px;font-weight:600;color:var(--accent-green)">' + t('va.on_shift_since') + ' ' + since + '</span>'
      + '<button class="btn btn-danger" style="font-size:11px;padding:4px 14px;margin-left:auto" onclick="clockOut();setTimeout(renderVA,500)">' + t('misc.clock_out') + '</button></div>';
  } else {
    clockHTML = '<button class="btn btn-primary" style="font-size:14px;padding:12px 28px;border-radius:var(--radius-lg)" onclick="clockIn();setTimeout(renderVA,500)">' + t('va.start_working') + '</button>';
  }
  ...
  container.innerHTML = ''
    ...
    + '<h3 style="font-size:15px;font-weight:700;color:var(--text-primary);margin-bottom:16px">' + t('va.tasks_today') + '</h3>'
    + (todayTasks.length > 0 ? '<div style="display:grid;gap:8px">' + todayTasks.map(function(tk) {
      ...
      return '<div style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:var(--bg-base);border-radius:10px;border-left:3px solid ' + pc + '">'
        + '<div style="flex:1"><strong style="font-size:13px">' + tk.title + '</strong>'
        + (tk.description ? '<div style="font-size:11px;color:var(--text-tertiary);margin-top:2px">' + tk.description + '</div>' : '') + '</div>'
        + '<select onchange="updateTaskStatus(' + tk.id + ',this.value);setTimeout(renderVA,300)" style="background:var(--bg-base);border:1px solid var(--border);color:var(--text);padding:4px 8px;border-radius:6px;font-size:11px;cursor:pointer;font-family:inherit">'
    ...
    }).join('') + '</div>' : '<div style="color:var(--text-tertiary);text-align:center;padding:20px">' + t('va.no_tasks_today') + '</div>')
    + '</div>'

    // Recurring tasks (daily checklist)
    + '<div class="panel" style="padding:20px">'
    + '<h3 style="font-size:15px;font-weight:700;color:var(--text-primary);margin-bottom:16px">' + t('va.daily_checklist') + '</h3>'
    + (dailyTasks.length > 0 ? '<div style="display:grid;gap:6px">' + dailyTasks.map(function(rt) {
      var done = rt.completed_today;
      return '<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:' + (done ? 'rgba(16,185,129,0.1)' : 'var(--bg-elevated)') + ';border-radius:8px;cursor:pointer;transition:all 0.15s" onclick="toggleRecurringTask(' + rt.id + ',' + (done ? 'false' : 'true') + ')">'
        ...
        + '</div>';
    }).join('') + '</div>' : '<div style="color:var(--text-tertiary);text-align:center;padding:20px">' + t('va.no_recurring') + '</div>')
    + (periodicTasks.length > 0 ? '<div style="margin-top:16px"><h4 style="font-size:12px;color:var(--text-tertiary);text-transform:uppercase;margin-bottom:8px">' + t('va.periodic_tasks') + '</h4>'
      + '<div style="display:grid;gap:6px">' + periodicTasks.map(function(rt) {
        ...
        return '<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:' + (done ? 'rgba(16,185,129,0.1)' : 'var(--bg-elevated)') + ';border-radius:8px;cursor:pointer" onclick="toggleRecurringTask(' + rt.id + ',' + (done ? 'false' : 'true') + ')">'
          + '<div style="width:20px;height:20px;border-radius:6px;border:2px solid ' + (done ? 'var(--green)' : 'var(--border)') + ';display:flex;align-items:center;justify-content:center;flex-shrink:0;background:' + (done ? 'var(--green)' : 'transparent') + '">' + (done ? '<span style="color:white;font-size:12px">✓</span>' : '') + '</div>'
          + '<span style="font-size:13px;' + (done ? 'text-decoration:line-through;opacity:0.5' : '') + '">' + rt.title + '</span>'
          + '<span style="font-size:10px;color:var(--text-tertiary);margin-left:auto;background:var(--bg-base);padding:2px 8px;border-radius:var(--radius-md)">' + freq + '</span></div>';
      }).join('') + '</div></div>' : '')
    + '</div>'
    + '</div>'
    ...
    // Messages widget
    + '<div class="panel" style="padding:20px"><h3 style="font-size:15px;font-weight:700;color:var(--text-primary);margin-bottom:16px">' + t('va.messages') + (unreadTotal > 0 ? ' <span style="background:var(--red);color:white;font-size:10px;padding:2px 8px;border-radius:var(--radius-md)">' + unreadTotal + '</span>' : '') + '</h3><div id="va-messages-widget"></div></div>'
    + '</div>'

    // Compensation widget
    + '<div class="panel" style="padding:20px;margin-bottom:20px">'
    + '<h3 style="font-size:15px;font-weight:700;color:var(--text-primary);margin-bottom:16px">' + t('va.my_compensation') + '</h3>'
    + renderVACompensation(compensation, payments)
    + '</div>'

    // All pending tasks
    + '<div class="panel" style="padding:20px">'
    + '<h3 style="font-size:15px;font-weight:700;color:var(--text-primary);margin-bottom:16px">' + t('va.all_tasks') + ' (' + myTasks.length + ')</h3>'
    + (myTasks.length > 0 ? '<div style="display:grid;gap:6px">' + myTasks.map(function(tk) {
      ...
      return '<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:' + (tk.status === 'in_progress' ? 'var(--yellow-bg)' : 'var(--bg-elevated)') + ';border-radius:8px;font-size:13px">'
        + '<span>' + tk.title + badge + '</span>'
        + (tk.deadline ? '<span style="color:var(--text-tertiary);font-size:11px;margin-left:auto">' + tk.deadline + '</span>' : '')
        + '</div>';
    }).join('') + '</div>' : '<div style="color:var(--text-tertiary);text-align:center;padding:12px">' + t('tasks.no_tasks') + '</div>')
    + '</div>';
...
function renderVACompensation(comp, payments) {
  ...
  return '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-bottom:16px">'
    + '<div style="background:var(--bg-base);padding:14px;border-radius:10px;text-align:center"><div style="font-size:12px;color:var(--text-tertiary)">' + t('va.pay_type') + '</div><div style="font-size:14px;font-weight:700;margin-top:4px">' + typeLabel + '</div></div>'
    + '<div style="background:var(--bg-base);padding:14px;border-radius:10px;text-align:center"><div style="font-size:12px;color:var(--text-tertiary)">' + t('va.rate') + '</div><div style="font-size:14px;font-weight:700;margin-top:4px">' + rateInfo + '</div></div>'
    + '<div style="background:var(--bg-base);padding:14px;border-radius:10px;text-align:center"><div style="font-size:12px;color:var(--text-tertiary)">' + t('va.hours_this_month') + '</div><div style="font-size:14px;font-weight:700;margin-top:4px">' + comp.hoursWorked + 'h</div></div>'
    + '<div style="background:var(--bg-base);padding:14px;border-radius:10px;text-align:center"><div style="font-size:12px;color:var(--text-tertiary)">' + t('va.due_this_month') + '</div><div style="font-size:20px;font-weight:800;color:var(--green);margin-top:4px">$' + comp.amountDue.toFixed(2) + '</div></div>'
    + '</div>'
    + (payments.length > 0 ? '<h4 style="font-size:12px;color:var(--text-tertiary);text-transform:uppercase;margin-bottom:8px">' + t('va.payment_history') + '</h4>'
      + '<div style="display:grid;gap:4px">' + payments.slice(0, 5).map(function(p) {
        var stColor = p.status === 'paid' ? 'var(--green)' : 'var(--yellow)';
        return '<div style="display:flex;justify-content:space-between;padding:8px 12px;background:var(--bg-base);border-radius:6px;font-size:12px">'
          + '<span>' + p.month + '</span><span style="color:var(--text-tertiary)">' + p.hours_worked + 'h</span>'
      ...
      }).join('') + '</div>' : '<div style="color:var(--text-tertiary);font-size:12px">' + t('va.no_payments') + '</div>');
}
...
async function loadVAContentLibrary() {
  ...
  try {
    ...
    if (items.length === 0) {
      el.innerHTML = '<div style="text-align:center;padding:20px"><div style="color:var(--text-tertiary);margin-bottom:12px">' + t('va.no_content') + '</div>'
    ...
    el.innerHTML = '<div style="margin-bottom:12px"><button class="btn btn-primary" style="font-size:11px;padding:4px 12px" onclick="showAddContentModal()">+ ' + t('va.add_content') + '</button></div>'
      + '<div style="display:grid;gap:6px;max-height:300px;overflow-y:auto">' + items.slice(0, 20).map(function(item) {
        ...
        var statusColor = item.status === 'ready' ? 'var(--green)' : item.status === 'published' ? 'white' : 'var(--text-tertiary)';
        var icon = item.content_type === 'video' ? '🎬' : '📸';
        return '<a href="' + item.external_url + '" target="_blank" style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:var(--bg-base);border-radius:8px;text-decoration:none;color:var(--text);transition:background 0.15s" onmouseover="this.style.background=\'var(--bg2)\'" onmouseout="this.style.background=\'var(--bg-elevated)\'">'
          ...
          + '<div style="font-size:11px;color:var(--text-tertiary)">' + (item.model_name || '-') + '</div></div>'
          + '<span style="font-size:10px;padding:2px 8px;border-radius:6px;background:' + statusBg + ';color:' + statusColor + '">' + item.status + '</span></a>';
      }).join('') + '</div>';
  } catch(e) { el.innerHTML = '<div style="color:var(--text-tertiary)">' + t('common.error') + '</div>'; }
}
...
async function loadVAMessages() {
  ...
  try {
    ...
    if (convs.length === 0) { el.innerHTML = '<div style="color:var(--text-tertiary);text-align:center;padding:20px">' + t('student.no_message') + '</div>'; return; }
    el.innerHTML = '<div style="display:grid;gap:6px;max-height:300px;overflow-y:auto">' + convs.map(function(c) {
      ...
      return '<div style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:' + (unread > 0 ? 'rgba(59,130,246,0.05)' : 'var(--bg-elevated)') + ';border-radius:8px;cursor:pointer" onclick="openVAChat(' + c.user_id + ',\'' + (c.display_name || '').replace(/'/g, "\\'") + '\')">'
        + '<div style="flex:1"><strong style="font-size:13px">' + (c.display_name || 'User #' + c.user_id) + '</strong>'
        + (c.last_message ? '<div style="font-size:11px;color:var(--text-tertiary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:200px">' + c.last_message + '</div>' : '') + '</div>'
  ...
  var html = '<div class="modal-overlay show" id="va-chat-modal" onclick="if(event.target===this)this.remove()">'
    ...
    + messages.map(function(m) {
      ...
      return '<div style="display:flex;justify-content:' + (isMine ? 'flex-end' : 'flex-start') + '">'
        + '<div style="max-width:75%;padding:8px 14px;border-radius:12px;font-size:13px;background:' + (isMine ? 'var(--accent)' : 'var(--bg-elevated)') + ';color:' + (isMine ? 'white' : 'var(--text)') + '">' + m.content + '</div></div>';
    }).join('')
    + '</div>'
...
async function renderVAAdmin(container) {
  ...
  container.innerHTML = ''
    ...
    + '<h3 style="font-size:16px;font-weight:700;color:var(--text-primary)">' + t('va.team_title') + ' (' + vas.length + ')</h3>'
    + '</div>'
    ...
    + (vas.length > 0 ? '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:14px;margin-bottom:20px">' + vas.map(function(v) {
      ...
      return '<div class="panel" style="padding:16px;border-left:3px solid ' + (online ? 'var(--green)' : 'var(--text-tertiary)') + '">'
        ...
        + '</div></div>';
    }).join('') + '</div>' : '<div style="color:var(--text-tertiary);text-align:center;padding:40px">' + t('va.no_va') + '</div>')

    // Content library admin view
    + '<div class="panel" style="padding:20px;margin-bottom:20px">'
    + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px"><h3 style="font-size:15px;font-weight:700;color:var(--text-primary)">' + t('va.content_library') + '</h3>'
    ...
    + '<div class="panel" style="padding:20px">'
    + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px"><h3 style="font-size:15px;font-weight:700;color:var(--text-primary)">' + t('va.recurring_tasks_admin') + '</h3>'
...
async function loadVARecurringAdmin() {
  ...
  try {
    ...
    if (tasks.length === 0) { el.innerHTML = '<div style="color:var(--text-tertiary);text-align:center;padding:16px">' + t('va.no_recurring') + '</div>'; return; }
    el.innerHTML = '<div style="display:grid;gap:6px">' + tasks.map(function(rt) {
      ...
      return '<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:var(--bg-base);border-radius:8px">'
        + '<span style="font-size:13px;font-weight:600;flex:1">' + rt.title + '</span>'
        + '<span style="font-size:10px;color:var(--text-tertiary);background:var(--bg-base);padding:2px 8px;border-radius:var(--radius-md)">' + freq + '</span>'
