...
async function renderFinance() {
  ...
  container.innerHTML = ''
    ...
    + '<div class="stat-card"><div class="stat-value" style="color:var(--text2);font-size:20px">$' + summary.previousMonth.revenue.toLocaleString(undefined, {maximumFractionDigits:0}) + '</div><div class="stat-label" style="display:flex;align-items:center;gap:6px;justify-content:center">' + t('finance.prev_month') + ' <span style="color:' + variationColor + ';font-weight:700;font-size:14px">' + variationIcon + ' ' + Math.abs(summary.variation) + '%</span></div></div>'
    + '<div class="stat-card"><div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:6px"><span style="color:var(--text2)">' + t('finance.objective') + '</span><span style="color:var(--accent);font-weight:700">' + objPct + '%</span></div>'
    + '<div style="height:8px;background:var(--bg);border-radius:4px;overflow:hidden;margin-bottom:6px"><div style="height:100%;width:' + objPct + '%;background:' + (objPct >= 100 ? 'var(--green)' : 'var(--accent)') + ';border-radius:4px;transition:width 0.5s"></div></div>'
    + '<div style="font-size:12px;color:var(--text3);text-align:center">$' + summary.objective.current.toLocaleString(undefined, {maximumFractionDigits:0}) + ' / $' + summary.objective.target.toLocaleString(undefined, {maximumFractionDigits:0}) + '</div></div>'
    ...
    + '<div style="display:grid;gap:8px">' + due.map(function(d) {
      return '<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:var(--bg3);border-radius:8px">'
    ...
    + '<h3 style="font-size:15px;font-weight:700;color:var(--accent2);margin-bottom:16px">' + t('finance.agency_commissions') + '</h3>'
    + '<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:16px">'
    + '<div style="background:var(--bg3);padding:14px;border-radius:10px;text-align:center"><div style="font-size:22px;font-weight:800;color:var(--accent)">$' + commissions.monthlyCommission.toLocaleString(undefined, {maximumFractionDigits:0}) + '</div><div style="font-size:12px;color:var(--text3)">' + t('finance.this_month_commission') + '</div></div>'
    + '<div style="background:var(--bg3);padding:14px;border-radius:10px;text-align:center"><div style="font-size:22px;font-weight:800;color:var(--green)">$' + commissions.yearlyCommission.toLocaleString(undefined, {maximumFractionDigits:0}) + '</div><div style="font-size:12px;color:var(--text3)">' + t('finance.yearly_commission') + '</div></div>'
    + '</div>'
    + (commissions.byModel.length > 0 ? '<div style="display:grid;gap:6px">' + commissions.byModel.map(function(cm) {
      return '<div style="display:flex;justify-content:space-between;padding:8px 12px;background:var(--bg3);border-radius:6px;font-size:13px"><span>' + cm.model_name + '</span><div><span style="color:var(--text3);margin-right:12px">$' + parseFloat(cm.revenue).toLocaleString(undefined, {maximumFractionDigits:0}) + ' rev</span><span style="color:var(--accent);font-weight:700">$' + parseFloat(cm.commission).toLocaleString(undefined, {maximumFractionDigits:0}) + ' comm</span></div></div>';
    }).join('') + '</div>' : '')
    + '</div>'
    ...
    + '<button class="btn" style="font-size:11px;padding:4px 12px;background:var(--bg3);color:var(--accent);border:none;cursor:pointer;border-radius:6px" onclick="exportPaymentsCSV()">📥 CSV</button>'
    ...
    + '<h3 style="font-size:15px;font-weight:700;color:var(--accent2)">' + t('finances.revenue_objectives') + ' — ' + curMonth + '</h3>'
    ...
    + objectives.filter(function(o) { return o.month === curMonth; }).map(function(o) {
      ...
      return '<div style="background:var(--bg3);padding:12px;border-radius:8px">'
        ...
        + '<div style="display:flex;align-items:center;gap:8px"><input type="number" value="' + o.current + '" onchange="updateRevenueObj(' + o.id + ',{current:this.value})" style="width:80px;background:var(--bg);border:1px solid var(--border);color:var(--text);padding:4px 8px;border-radius:6px;font-size:12px;font-family:inherit;text-align:right">'
        + '<span style="font-size:13px;color:var(--text3)">/ $' + parseFloat(o.target).toFixed(0) + '</span><span style="font-size:12px;font-weight:700;color:' + (pct >= 100 ? 'var(--green)' : 'var(--accent)') + '">' + pct + '%</span></div></div>'
        + '<div style="height:6px;background:var(--bg);border-radius:3px;overflow:hidden"><div style="height:100%;width:' + pct + '%;background:' + (pct >= 100 ? 'var(--green)' : 'var(--accent)') + ';border-radius:3px"></div></div></div>';
...
function renderFinanceCharts(monthly, breakdown) {
  ...
  var chartColors = ['#A855F7', '#22D3EE', '#10B981', '#F472B6', '#F59E0B', '#EF4444', '#6366F1', '#84CC16'];
  var months = [];
  var modelSet = {};
  ...
  if (ctx3) {
    ...
    if (ppv + tips > 0) {
      ...
      financeChartBySource = new Chart(ctx3.getContext('2d'), {
        ...
        data: {
          labels: ['PPV', 'Tips'],
          datasets: [{ data: [ppv, tips], backgroundColor: ['#A855F7', '#22D3EE'], borderWidth: 0 }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { color: '#9ca3af', font: { size: 10 }, boxWidth: 12 } } } }
      });
    }
