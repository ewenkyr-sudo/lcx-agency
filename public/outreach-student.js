...
function renderSOBulkBar(studentUserId) {
  ...
  bar.innerHTML = '<div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;padding:12px 16px;background:var(--bg3);border:1px solid var(--accent);border-radius:10px;margin-bottom:16px">'
    ...
    + '<select id="so-bulk-script-' + studentUserId + '" class="form-input" style="max-width:200px;font-size:12px;padding:6px 8px"><option value="">' + t('outreach.apply_script') + '</option>' + scriptOpts + '</select>'
    + '<select id="so-bulk-account-' + studentUserId + '" class="form-input" style="max-width:200px;font-size:12px;padding:6px 8px"><option value="">' + t('outreach.apply_ig') + '</option>' + accountOpts + '</select>'
    + '<button class="btn btn-primary" style="padding:6px 14px;font-size:12px" onclick="applySOLeadsBulk(' + studentUserId + ')">' + t('common.apply') + '</button>'
    + '<button class="btn" style="background:var(--bg2);color:var(--text2);border:none;padding:6px 14px;font-size:12px;cursor:pointer" onclick="clearSOSelection(' + studentUserId + ')">' + t('common.deselect') + '</button>'
    + '</div>';
}
...
async function applySOLeadsBulk(studentUserId) {
  ...
  showToast(ids.length + ' ' + t('student.leads_updated'), 'success');
...
async function initOutreachStudentSections() {
  ...
  assignedStudents.forEach(function(a) {
    ...
    navItem.innerHTML = '<span class="nav-icon">📨</span> ' + t('so.outreach_for') + ' ' + a.student_name;
...
async function renderStudentOutreachForAssistant(studentUserId, studentName) {
  ...
  c.innerHTML = '<div class="page-header"><div><div class="page-title">' + t('so.outreach_for') + ' ' + studentName + '</div><div class="page-subtitle">' + t('so.leads_for') + ' ' + studentName + '</div></div>'
    + '<div class="header-actions" style="display:flex;gap:8px"><button class="btn btn-primary" onclick="showSOLeadForm(' + studentUserId + ')">' + t('common.new_lead') + '</button>'
    + '<button class="btn" style="background:var(--bg3);color:var(--text2);border:none;cursor:pointer" onclick="showSOOptionsManager(' + studentUserId + ')">' + t('outreach.my_options') + '</button></div></div>'
    + '<div class="stats-grid" style="margin-bottom:20px">'
    + '<div class="stat-card"><div class="stat-value" style="color:var(--blue)">' + (statsRes.dm_sent_today || 0) + '</div><div class="stat-label">' + t('outreach.dms_today') + '</div></div>'
    + '<div class="stat-card"><div class="stat-value">' + (statsRes.dm_sent || 0) + '</div><div class="stat-label">' + t('outreach.dms_total') + '</div></div>'
    + '<div class="stat-card"><div class="stat-value" style="color:var(--yellow)">' + (statsRes.talking_warm || 0) + '</div><div class="stat-label">' + t('outreach.talking_warm') + '</div></div>'
    + '<div class="stat-card"><div class="stat-value" style="color:var(--accent2)">' + (statsRes.reply_rate || 0) + '%</div><div class="stat-label">' + t('outreach.reply_rate') + '</div></div>'
    + '</div>'
    ...
    + ['all','to-send','sent','talking-cold','talking-warm','call-booked','signed'].map(function(f) {
      ...
      var label = f === 'all' ? t('common.all') : (leadStatusColors[f] ? leadStatusColors[f].label : f);
    ...
    + '</div>'
    + '<div style="margin-bottom:16px"><input type="text" id="so-search-' + studentUserId + '" class="form-input" placeholder="' + t('outreach.search_placeholder') + '" oninput="debouncedRenderSOLeadTable(' + studentUserId + ')" style="max-width:350px"></div>'
...
function renderSOLeadTable(studentUserId) {
  ...
  tbody.innerHTML = filtered.map(function(l, idx) {
    ...
    return '<tr><td data-label="" style="width:30px"><input type="checkbox" class="so-lead-cb" data-id="' + l.id + '" ' + checked + ' onchange="toggleSOLeadSelection(' + studentUserId + ',' + l.id + ',this.checked)"></td>'
      ...
      + '<td data-label="' + t('so.modified_by') + '" class="mc-half" style="font-size:11px;color:var(--accent2)">' + (l.modified_by_name || l.added_by_name || '-') + '</td>'
  ...
  }).join('') || '<tr><td colspan="10">' + emptyStateHTML('search', t('so.no_lead')) + '</td></tr>';
  renderSOBulkBar(studentUserId);
}
...
function showSOLeadForm(studentUserId) {
  ...
  wrap.innerHTML = '<div class="panel" style="padding:20px;margin-bottom:20px">'
    ...
    + '<div><label style="font-size:12px;color:var(--text2);display:block;margin-bottom:4px">' + t('so.ig_link_label') + '</label><input type="text" id="so-iglink-' + studentUserId + '" class="form-input" placeholder="' + t('so.paste_link') + '" oninput="autoFillUsername(this.value)" onpaste="var s=' + studentUserId + ';setTimeout(function(){autoFillUsername(document.getElementById(\'so-iglink-\'+s).value)},100)"></div>'
    + '<div><label style="font-size:12px;color:var(--text2);display:block;margin-bottom:4px">' + t('so.username_auto') + '</label><input type="text" id="lead-username" class="form-input" placeholder="' + t('so.auto_fill') + '" style="opacity:0.7"></div>'
    + '<div><label style="font-size:12px;color:var(--text2);display:block;margin-bottom:4px">' + t('common.type') + '</label><select id="so-type-' + studentUserId + '" class="form-input"><option value="">-- ' + t('common.type') + ' --</option>' + Object.entries(leadTypeColors).map(function(e) { return '<option value="' + e[0] + '">' + e[1].label + '</option>'; }).join('') + '</select></div>'
    + '<div><label style="font-size:12px;color:var(--text2);display:block;margin-bottom:4px">' + t('outreach.script') + '</label>' + sel('so-script-' + studentUserId, 'script', t('outreach.script')) + '</div>'
    + '<div><label style="font-size:12px;color:var(--text2);display:block;margin-bottom:4px">' + t('outreach.account') + '</label>' + sel('so-account-' + studentUserId, 'account', t('outreach.account')) + '</div>'
    + '<div><label style="font-size:12px;color:var(--text2);display:block;margin-bottom:4px">' + t('common.notes') + '</label><input type="text" id="so-notes-' + studentUserId + '" class="form-input" placeholder="' + t('student.notes_placeholder') + '"></div>'
    + '</div><div style="margin-top:12px;display:flex;gap:10px"><button class="btn btn-primary" onclick="addSOLead(' + studentUserId + ')">' + t('common.add') + '</button><button class="btn" style="background:var(--bg3);color:var(--text2);border:none;cursor:pointer" onclick="document.getElementById(\'so-form-wrap-' + studentUserId + '\').innerHTML=\'\'">' + t('common.cancel') + '</button></div></div>';
}

async function addSOOption(studentUserId, optType, selectId) {
  var labels = { script: 'script', account: t('so.ig_account_label'), type: 'type' };
  var value = await showPromptModal(t('common.new_prefix') + ' ' + (labels[optType] || optType), 'Ex: ' + (optType === 'script' ? 'Script DM v2' : optType === 'account' ? '@moncompte' : 'Type'));
  if (!value) return;
...
async function addSOLead(studentUserId) {
  ...
  if (!username) return showToast(t('so.paste_ig'), 'error');
  ...
  if (dup) return showToast(t('so.lead_exists') + ' (' + dup.status + ')', 'error');
  ...
  if (res.ok) {
    ...
    renderSOLeadTable(studentUserId);
    showToast(t('so.lead_added'), 'success');
  } else { try { var e = await res.json(); showToast(e.error || t('common.error'), 'error'); } catch(err) { showToast(t('toast.error_server'), 'error'); } }
}
...
function showSOOptionsManager(studentUserId) {
  ...
  var opts = (studentOutreachData[studentUserId] || {}).options || {};
  var labels = { type: t('so.types_label'), script: t('so.scripts_label'), account: t('so.ig_accounts_label') };
  wrap.innerHTML = '<div class="panel so-options-mgr" style="padding:20px;margin-bottom:20px">'
    + '<h3 style="font-size:15px;font-weight:700;margin-bottom:16px;color:var(--accent2)">' + t('so.shared_options') + '</h3>'
    ...
    + Object.entries(labels).map(function(e) {
      ...
      return '<div style="background:var(--bg3);padding:14px;border-radius:10px"><strong style="font-size:13px;display:block;margin-bottom:10px">' + label + '</strong>'
        + (list.length === 0 ? '<div style="color:var(--text3);font-size:12px">' + t('so.no_option') + '</div>' : '')
        ...
        + '<div style="display:flex;gap:6px;margin-top:10px"><input type="text" id="so-newopt-' + key + '-' + studentUserId + '" class="form-input" style="font-size:12px;padding:6px 8px;flex:1" placeholder="' + t('outreach.add_option') + '"><button class="btn btn-primary" style="padding:6px 10px;font-size:11px" onclick="addSOOptionFromMgr(' + studentUserId + ',\'' + key + '\')">+</button></div></div>';
    }).join('')
    + '</div><div style="margin-top:12px"><button class="btn" style="background:var(--bg3);color:var(--text2);border:none;cursor:pointer" onclick="document.getElementById(\'so-form-wrap-' + studentUserId + '\').innerHTML=\'\'">' + t('common.close') + '</button></div></div>';
}
...
async function addSOOptionFromMgr(studentUserId, optType) {
  ...
  if (res.ok) {
    var opt = await res.json();
    studentOutreachData[studentUserId].options[optType].push(opt);
    showToast('"' + value.trim() + '" ' + t('outreach.option_added'), 'success');
...
async function deleteSOOption(studentUserId, optId) {
  ...
  showToast(t('so.deleted'), 'success');
  showSOOptionsManager(studentUserId);
}
